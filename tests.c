...
//THIS TEST CASE WAS EDITED SO THAT THE RESIZE FUNCTION WOULD BE CALLED
//KEYS H-M were added, and '5' was changed to '11' in testCase argument
...
void testSingleOver(CuTest* test)
{
    ...
    HashLink links[] = {
        ...
        { .key = "f", .value = 3, .next = NULL },
        { .key = "g", .value = 4, .next = NULL },
        { .key = "h", .value = 5, .next = NULL },
        { .key = "i", .value = 6, .next = NULL },
        { .key = "j", .value = 7, .next = NULL },
        { .key = "k", .value = 8, .next = NULL },
        { .key = "l", .value = 9, .next = NULL },
        { .key = "m", .value = 10, .next = NULL }
    };
    const char* notKeys[] = { "b", "e", "n" };
    testCase(test, links, notKeys, 11, 3, 1);
}
